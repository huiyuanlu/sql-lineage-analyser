/*
PrestoSQL grammar file based on SqlBase.g4.

For python target, the following symbols are renamed to avoid conflicts.
...
statement
    ...
    | ALTER TABLE (IF EXISTS)? r_from=qualifiedName
        RENAME TO to=qualifiedName                                     #renameTable
    | ALTER TABLE (IF EXISTS)? tableName=qualifiedName
        RENAME COLUMN (IF EXISTS)? r_from=identifier TO to=identifier    #renameColumn
    | ALTER TABLE (IF EXISTS)? tableName=qualifiedName
...
columnDefinition
    : identifier r_type (NOT NULL)? (COMMENT string)? (WITH properties)?
    ;
...
primaryExpression
    ...
    | qualifiedName '(' (setQuantifier? expression (',' expression)*)?
        (ORDER BY sortItem (',' sortItem)*)? ')' r_filter? (nullTreatment? over)?           #functionCall
...
interval
    : INTERVAL sign=(PLUS | MINUS)? string r_from=intervalField (TO to=intervalField)?
    ;
...
r_type
    ...
    | INTERVAL r_from=intervalField TO to=intervalField
    ;
